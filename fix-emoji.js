...
const filePath = path.join(__dirname, 'src', 'PlannerPage.tsx');
const content = fs.readFileSync(filePath, 'utf8');
const lines = content.split('\n');

// Find header end (line 16, 0-indexed 15 = closing `};` of type Room)
// Find component start
const compStartIdx = lines.findIndex(l => l.startsWith('export default function PlannerPage'));
console.log('Component starts at index:', compStartIdx, '(line', compStartIdx + 1 + ')');

const header = lines.slice(0, 16).join('\n');
const component = lines.slice(compStartIdx).join('\n');
...
const emojiLine = check.split('\n').find(l => l.includes('emoji:') && l.includes('living') || (l.includes('emoji:') && check.split('\n').indexOf(l) < 40));
console.log('Emoji sample:', check.split('\n')[21]);
